...
if measures_file is not None and series_position_file is not None:
    ...
    if st.button('Executar Otimização'):
        ...
        def plot_unbalanced_current(estados):
            # Extrair dados para o gráfico
            permutacoes = [estado['permutacao'] for estado in estados]
            unbalanced_currents = [estado['unbalanced_current'] for estado in estados]

            # Criar o gráfico com linhas e marcadores
            fig = px.line(
                x=permutacoes,
                y=unbalanced_currents,
                labels={'x': 'Permutação', 'y': 'Corrente de Desbalanceamento (A)'},
                title='Corrente de Desbalanceamento por Permutação',
                markers=True,  # Adicionar marcadores nos pontos
                template='plotly_white'
            )

            # Atualizar layout para eixo X discreto, formatação do eixo Y e aumentar o tamanho da fonte
            fig.update_layout(
                xaxis=dict(
                    tickmode='linear',  # Eixo X com ticks em números inteiros
                    tick0=0,
                    dtick=1,
                    title_font=dict(size=24)  # Aumentar a fonte do título do eixo X
                ),
                yaxis=dict(
                    tickformat='.4f',  # Quatro casas decimais no eixo Y
                    title_font=dict(size=24)  # Aumentar a fonte do título do eixo Y
                ),
                title=dict(
                    font=dict(size=24)  # Aumentar a fonte do título do gráfico
                ),
                font=dict(
                    size=20  # Aumentar tamanho da fonte para outros elementos
                )
            )

            # Adicionar anotações para cada permutação, com deslocamento vertical
            for i, permutacao in enumerate(permutacoes):
                fig.add_annotation(
                    x=permutacao,
                    y=unbalanced_currents[i] + 0.015,  # Ajustar posição vertical para deslocar o texto acima
                    text=f"{unbalanced_currents[i]:.4f}",  # Valor com 4 casas decimais
                    showarrow=False,  # Não exibir seta
                    font=dict(size=18),  # Tamanho da fonte da anotação
                    align='center'
                )
        ...
        # Exibir o gráfico e permitir download
        if resultados['permutations']:
            st.subheader('Corrente de Desbalanceamento por Permutação')
            fig = plot_unbalanced_current(resultados['estados_por_permutacao'])
            st.plotly_chart(fig)

            # Botão para salvar o gráfico em PNG
            buffer = io.BytesIO()
            fig.write_image(buffer, format='png', width=1280, height=720, scale=1)  # Alta resolução
            buffer.seek(0)

            st.download_button(
                label="Baixar Gráfico em PNG",
                data=buffer,
                file_name="corrente_desbalanceamento.png",
                mime="image/png",
            )
